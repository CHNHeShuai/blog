    ...
    <div class="container-fluid">
        ...
        <div class="ad_content">
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                <c:forEach items="${indexnewsList}" var="indexnews" varStatus="status">
                    <tr>
                        <td>${status.count}</td>
                        <td>${indexnews.title}</td>
                        <td><img style="width: 50px;height: 50px;" src="${pageContext.request.contextPath}/${indexnews.image}"></td>
                        <td>${indexnews.createtime}</td>
                        <td>${indexnews.des}</td>
                        <td>${indexnews.pageview}</td>
                        <td>${indexnews.click}</td>
                        <td>
                            <button type="button" onclick="findIndexnews(${indexnews.id})" style="padding: 6px 12px;font-size: 12px" class="btn btn-primary btn-lg" data-toggle="modal" data-target="#edit">
                                修改
                            </button>
                            <a onclick="deleteIndexnews(${indexnews.id})" href="javascript:;" class="btn btn-sm btn-danger">删除</a>
                        </td>
                        <script>
                            function deleteIndexnews(id) {
                                layer.msg('确定删除该首页新闻?', {
                                    time: 0 //不自动关闭
                                    ,btn: ['确定', '取消']
                                    ,yes: function(){
                                        window.location.href="/deleteIndexnews?id="+id;
                                    }
                                });
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form method="post" enctype="multipart/form-data" action="/editIndexnews">
